...
export default function AuthContextProvider({ children }) {
  const [currentUser, setCurrentUser] = useState({
    ...
    email:"",
    displayName:"",
    photoURL:"https://cdn.freebiesupply.com/logos/large/2x/ck-logo-svg-vector.svg"
  })

  // * this useEffect is updating the [currentUser] state each time the user login & logout
  useEffect(() => {
    const unsubscribe = onAuthStateChanged(auth, user => {
      setCurrentUser(user ? user : {uid:1234, email:"email",displayName:"name",photoURL:"https://cdn.freebiesupply.com/logos/large/2x/ck-logo-svg-vector.svg"});
    })
    return () => {
      unsubscribe()
    }
